...
export function RouteMap({ onNavigate, onSelectSenior }: RouteMapProps) {
  ...
  const handleOptimizedRoute = () => {
    const addresses = seniors.map((s) => s.address).join("|")
  ...
  const completedCount = seniors.filter((s) => getDeliveryStatus(s.id).isDelivered).length
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-4 space-y-4">
        {/* Route Summary - Start directly with this section */}
        <Card>
